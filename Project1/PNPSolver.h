...
public:
	...
	/***********************位姿估计所用特征点**************************/
	vector<Point3f> Points3D;//存储四个点的世界坐标
	vector<Point2f> Points2D;//存储四个点的图像坐标
	...
	Point3f get_distance(vector <Point2f> points);
private:

	Mat camera_matrix;//内参数矩阵
	Mat distortion_coefficients;//畸变系数
	...
	int Solve(int method = CV_EPNP);//    CV_ITERATIVE  CV_P3P   CV_EPNP
};
